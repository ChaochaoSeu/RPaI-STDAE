...
class OutputBlock(nn.Module):
    ...
    def __init__(self, Ko, last_block_channel, channels, end_channel, n_vertex, act_func, bias, droprate):
        ...
        self.silu = nn.SiLU()
        self.dropout = nn.Dropout(p=droprate)
        self.fc_his_t = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, channels[0]), nn.ReLU())
        self.fc_his_s = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, channels[0]), nn.ReLU())

    def forward(self, x, hidden_states_t, hidden_states_s):
        ...
        if hidden_states_t is not None and hidden_states_s is not None:
            ...
            # 只有 temporal
            fused_state = self.fc_his_t(hidden_states_t)

        elif hidden_states_s is not None:
            # 只有 spatial
            fused_state = self.fc_his_s(hidden_states_s)

        if hidden_states_t is not None:
            fused_state = fused_state.unsqueeze(1)
            x = x + fused_state
