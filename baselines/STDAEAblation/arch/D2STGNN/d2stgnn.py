...
class D2STGNN(nn.Module):
    ...
    def __init__(self, **model_args):
        ...
        self._forecast_dim = 64
        self._output_hidden = 128
        ...
        self.fc_his_t = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, 4 * 64), nn.ReLU())
        self.fc_his_s = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, 4 * 64), nn.ReLU())
    ...
    def _prepare_inputs(self, X):
        ...
        time_feature = X[:, :, :, num_feat] # 映射到(0,1)
        day_feature = X[:, :, :, num_feat + 1] # 映射到(0,1)
        ...
        T_i_D = self.T_i_D_emb[(time_feature * self._time_in_day_size).type(torch.LongTensor)]

        # [B, L, N, d]
        D_i_W = self.D_i_W_emb[(day_feature * self._day_in_week_size).type(torch.LongTensor)]
        # traffic signals
        X = X[:, :, :, :num_feat]
    ...
    def forward(self, history_data, hidden_states_t, hidden_states_s):
        ...
        if hidden_states_t is not None and hidden_states_s is not None:
            ...
            # 只有 temporal
            hidden_states_t = self.fc_his_t(hidden_states_t)
            fused_state = hidden_states_t
        elif hidden_states_s is not None:
            # 只有 spatial
            hidden_states_s = self.fc_his_s(hidden_states_s)
            fused_state = hidden_states_s

        if hidden_states_t is not None:

            forecast_hidden = forecast_hidden + fused_state.reshape(B, 12, 4, 64).transpose(1, 2)
