...
class STDAELSTM(nn.Module):

    def __init__(self, dataset_name, pre_trained_tae_path, pre_trained_sae_path, stdae_args, lstm_args,
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int,
        ...
        """
        # data
        long_history_data = history_data[:, :, :, :4]  # [B, L * P, N, C=4] # 这里要选取原始数据的主线交通流量、速度时间序列特征

        short_term_history = history_data[:, -self.short_term_len:, :, :] # [B, L, N, D]
