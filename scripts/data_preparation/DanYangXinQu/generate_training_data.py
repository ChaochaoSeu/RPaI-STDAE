...
import numpy as np
from generate_adj_mx import generate_adj_DanYangXinQu as generate_adj

# Hyperparameters
dataset_name = 'DanYangXinQu'
frequency = 10
steps_per_day = 1440 // frequency  # Number of time steps per day

data_file_path = f'datasets/InterchangeData/{dataset_name}/{dataset_name}_{frequency}min.npz'
graph_file_path = f'datasets/InterchangeData/{dataset_name}/adj_{dataset_name}.pkl'
output_dir = f'datasets/{dataset_name}_{frequency}min'
target_channel = list(range(10))  # Target traffic flow channel
...
regular_settings = {
    'INPUT_LEN': 12,
    'OUTPUT_LEN': 12,
    'TRAIN_VAL_TEST_RATIO': [17/23, 3/23, 3/23],
...
def load_and_preprocess_data():
    ...
    print(f'Parking time series shape: {data.shape}')
...
def add_temporal_features(data):
    ...
    print('处理后data.shape',data_with_features.shape)
