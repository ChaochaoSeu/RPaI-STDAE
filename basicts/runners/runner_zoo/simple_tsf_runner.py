...
class SimpleTimeSeriesForecastingRunner(BaseTimeSeriesForecastingRunner):
    ...
    def postprocessing(self, input_data: Dict) -> Dict:
        ...
        if self.scaler is not None and self.scaler.rescale:
            input_data['prediction'] = self.scaler.inverse_transform(input_data['prediction'], 'target_channel')
            input_data['target'] = self.scaler.inverse_transform(input_data['target'], 'target_channel')
            input_data['inputs'] = self.scaler.inverse_transform(input_data['inputs'], 'selected_channel')
    ...
    def forward(self, data: Dict, epoch: int = None, iter_num: int = None, train: bool = True, **kwargs) -> Dict:
        ...
        selected_history_data = self.select_input_features(history_data)
        future_data_4_dec = self.select_target_features(future_data)

        if not train:
            # For non-training phases, use only temporal features
            future_data_4_dec[..., self.target_features] = torch.empty_like(future_data_4_dec[..., self.target_features])

        # Forward pass through the model
        model_return = self.model(history_data=selected_history_data, future_data=future_data_4_dec,
        ...
        if 'inputs' not in model_return:
            model_return['inputs'] = self.select_input_features(history_data)
